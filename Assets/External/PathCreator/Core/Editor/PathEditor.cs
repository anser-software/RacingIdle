        ...
        void DrawBezierPathInspector () {
            using (var check = new EditorGUI.ChangeCheckScope ()) {
                ...
                if (data.showPathOptions) {
                    ...
                    pathToLoad = EditorGUILayout.IntField(new GUIContent("Load Path Index"), pathToLoad);

                    if (GUILayout.Button("Load Path"))
                    {
                        Undo.RecordObject(creator, "Reset Path");

                        var roadBezierPathJson = Resources.Load<TextAsset>("Roads/Road_" + pathToLoad.ToString());

                        if (roadBezierPathJson == null)
                        {
                            Debug.LogError(string.Format("Road with index {0} not found", pathToLoad.ToString()));
                            return;
                        }
                        else
                        {
